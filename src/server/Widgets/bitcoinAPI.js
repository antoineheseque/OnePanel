...
router.get("/upCours", (req, res) => {
    axios.get("https://api.blockchain.info/charts/market-price?timespan=1days&rollingAverage=1hours&format=json").then(function(r){
        ...
        sql.request(`INSERT \`Bitcoins\` SET valeur='${coursDuJour} SET date='${date}`).then(function(result){
...
router.post("/getcours", (req, res)=>{
    ...
    sql.request(`SELECT valeur from \`Bitcoins\` ORDER BY id DESC LIMIT 7 `).then(function(result){
        res.json.clear();
        for(var k=0;k<7;k++){
